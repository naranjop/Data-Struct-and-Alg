...
def splitArrayHelper(start, nums, sumA = 0, sumB = 0):
    if start >= len(nums):
        return (sumA == sumB)
    return splitArrayHelper(start + 1, nums, sumA + nums[start], sumB) or splitArrayHelper(start + 1, nums, sumA, sumB + nums[start])
