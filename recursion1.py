...
def bunnyEars2(bunnies):
    if bunnies < 1:
        ...
        return 7
    return bunnyEars2(bunnies - 1) + bunnyEars2(bunnies - 2) - bunnyEars2(bunnies - 3)
...
def count8(n):
    if int(str(n)[-1]) == 8:
        if len(str(n)) == 1:
            ...
            if int(str(n)[-2]) == 8:
                return 2 + count8(int(str(n)[:-1]))
            else:
                return 1 + count8(int(str(n)[:-1]))
    if len(str(n)) == 1:
        return 0
    return count8(int(str(n)[:-1]))
...
def countX(string):
    if len(string) < 1:
        ...
        if string == 'x':
            ...
            return 0
    return countX(string[:-1]) + countX(string[-1])
...
def countHi(string):
    ...
    if len(string) == 2:
        if string == 'hi':
            return 1
        return 0
    return countHi(string[-2:]) + countHi(string[:-1])
...
def changeXY(string):
    if len(string) < 1:
        ...
        if string == 'x':
            return 'y'
        return string
    return changeXY(string[:-1]) + changeXY(string[-1])
...
def changePi(string):
    if len(string) < 2:
        ...
        return changePi(string[:-2]) + '3.14'
    return changePi(string[:-1]) + string[-1]
...
def noX(string):
    if string == '':
        ...
        if string == 'x':
            return ''
        return string
    return noX(string[:-1]) + noX(string[-1])
...
def allStar(string):
    if len(string) < 2:
        return string
    return allStar(string[0]) + '*' + allStar(string[1:])
...
def pairStar(string):
    if len(string) < 2:
        ...
        return pairStar(string[0]) + '*' + pairStar(string[1:])
    return pairStar(string[0]) + pairStar(string[1:]) 
...
def countAbc(string):
    if len(string) < 3:
        return 0
    elif string[0:3] == 'abc' or string[0:3] == 'aba':
...
def count11(string):
    if len(string) < 2:
        return 0
    elif string[0:] == '11' or string[0:2] == '11':
...
def countHi2(string):
    if string == 'hi':
        ...
        return 0
    elif string[-2:] == 'hi' and string[-3] != 'x':
...
def strCount(string, sub):
    ...
    if string == sub:
        ...
        return 0
    elif string[:length] == sub:
        return 1 + strCount(string[length:], sub)
...
def strCopies(string, sub, n):
    length = len(sub)
    if string == sub and n <= 1:
        return True
    elif length > len(string):
        return False
    elif (string[:length] == sub and strCopies(string[1:], sub, n - 1)) or (string[:length] == sub and n <= 1):
        return True
...
#    elif length > len(string):
#        return 0
#    elif strDist(string[:length], sub) != 0 and strDist(string[-length:], sub) != 0:
#       return len(string)
#    elif strDist(string[:length], sub) != 0:
#        return len(string[:length]) + strDist(string[length:], sub)
#    elif strDist(string[:length], sub) != 0 and strDist(string[-length:], sub) != 0:
